...
do ID_PREC=7,7,-5
 do DP_Depth_nr=2,2,1
  do IRHW = 1,3,2
    DP_Depth=DP_Depth_ID(DP_Depth_nr)
   write(Dp_depth_str,*) DP_Depth

  !ID_PREC=0
EXP_NAME= 'data/sheusp_IMPR_SP_FRPPreAll_FRPLap_SPr0V2Upd_RPxAx_DP'//trim(adjustl(Dp_depth_str))//'_L2Exit_1M3_dt200_res4'
...
double precision :: PMB_dp(n1,n2), PMP0_dp(n1, n2), p_true_dp(n1, n2),p0_true_dp(n1, n2), b_true_dp(n1, n2) 
double precision :: p_mean_dp(n1,n2), p_T_all(n1,n2) 
...
p_T_rp(:,:)=p_T(:,:)
 ...
 DO J=1,n2
   DO I=1,n1
    PMB_dp(I, J)= p_dp(I,J)+p_T_all(I,J)-b(I,J)
    PMP0_dp(I,J)= p_dp(I,J)+p_T_all(I,J)-p0(I,J)
   enddo
 enddo
...
RPE_DEFAULT_SBITS =23 !52
 ! double precision residual  
  CALL PRFORC_ABS_dp(p_dp(:,:)+dble(p_T_all(:,:)),pfx_dp,pfy_dp,dble(pb),dble(p0), &
       &      dble(E1(:,:)),dble(E2(:,:)),dble(HX),dble(HY),dble(COR), &
       &      n1,n2,IP,dble(GC1),dble(GC2),dble(Alp_REL),1,1)
  call diver_dp(r_HP_dp,pfx_dp,pfy_dp,dble(hx),dble(hy),dble(s),n1,n2,ip,-1)
...
!!! begin true residual
   ...
   DO J=1,n2
     DO I=1,n1
       r_true_dp(I,J)=0.5d0*r_true_dp(I,J)-(dble(p_T_dp(I,J))+dble(b_true(I,J)))
       ! write(*,*), i, J, P(i,J)
     enddo
   enddo
   err_true_dp=0.0d0
...
!p_T(:,:)=0.0d0
p_T_all(:,:)=p_T_all(:,:)+p_T_dp(:,:)


!   p_mean_rp(:,:)=p_mean_rp(:,:)+p_T(:,:)
! p_mean_rp(:,n2+(1-DP_Depth_rpe):n2)=p_mean_rp(:,n2+(1-DP_Depth_rpe):n2)+p_T(:,n2+(1-DP_Depth_rpe):n2)
! p_pert_rp(:,DP_Depth_rpe+1:n2+(1-DP_Depth_rpe)-1)=p_pert_rp(:,DP_Depth_rpe+1:n2+(1-DP_Depth_rpe)-1)+ &
!                     & p_T(:,DP_Depth_rpe+1:n2+(1-DP_Depth_rpe)-1)

b(:,:)=dble(b(:,:))+dble(p_T_all(:,:))
end do
p_T(:,:)=p_T_all(:,:)
!write(*,*) niter
!  200
