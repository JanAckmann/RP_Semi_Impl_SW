...
INTEGER, PARAMETER :: NLON=(256)+2,NLAT=128
...
type(rpe_var):: PT_mean_rp,PT_zmean_rp(N,M), PT_pert_rp(N,M), PT_sum_rp(N,M)
type(rpe_var):: PT_mean10_rp,PT_zmean10_rp(N,M), PT_pert10_rp(N,M)
RPE_DEFAULT_SBITS =23 !52
PT_mean10_rp%sbits=10
PT_pert10_rp(:,:)%sbits=10
PT_zmean10_rp(:,:)%sbits=10
PT_mean_rp%sbits=23
PT_pert_rp(:,:)%sbits=23
...
do ID_PREC=7,7,-5
 do DP_Depth_nr=1,2,1
  ...
  !ID_PREC=0
EXP_NAME= 'data/sheusp_IMPR_SP_FRPPreAll_FRPLap_SPr0V2_RPxAx_DP'//trim(adjustl(Dp_depth_str))//'_L2Exit_1M3_dt400_res2'
...
mpfl=999999
elseif(IRHW==1) then
NT = 6480/2 !  6376 !int(6376*(200.0/240.0)) !12960  
NPRINT =216/2 !797 !797 !int(797*(200.0/240.0)) !864 !DATA NT,NPRINT/12096,864/
...
elseif (IRHW.EQ.3)then
  ...
  CALL SMOOTHTOP(P0_512x256,P0,N,M)
else
  P0(:,:)=0.0
endif
P0_HP(:,:)=P0(:,:)
...
!        !endif
      do I=1,N
        ...
        PT_zmean_rp(I,J)=PT_HP(I,J)-dble(PT_mean_rp%val)
        PT_zmean10_rp(I,J)=PT_HP(I,J)-dble(PT_mean10_rp%val)
      end do        
      enddo
      !        read(*,*)
!
      DO J=1,M
        DO I=1,N
            PT_pert_rp(I,J)= PT_HP(I,J)  -dble( PT_mean_rp%val)  -dble( PT_zmean_rp(I,J)%val)
            PT_pert10_rp(I,J)= PT_HP(I,J)-dble( PT_mean10_rp%val)-dble( PT_zmean10_rp(I,J)%val)
...
IF(IANAL.EQ.0) THEN
  DO KT=1,NT
    ...
    IPRINT=0
    IF(KT/NPRINT*NPRINT.EQ.KT .OR. KT==6376/2) IPRINT=1
    !if (IPRINT==1) then
...
!  CALL  GCR_PRE10_RP(PT_HP,F1(:,:,0),F2(:,:,0),HX,HY,S,S_full,F1(:,:,1),F2(:,:,1), &
!       &      PD(:,:),E1(:,:,0),E2(:,:,0),COR,IP, &
...
!       &  EXP_NAME, iprint, num_of_bits, DP_Depth, Alp_REL, PT_ZMEAN10_RP, PT_PERT10_RP)
  ...
  &      niter,nitsm,icount,error,  PD_T, sum_time, sum_lp_time, ID_PREC,.FALSE., save_time,&
       &      TIME, codesignQ, IRHW, X, Y, Exit_Cond,               &
       &  EXP_NAME, iprint, num_of_bits, DP_Depth, Alp_REL,PT_mean_rp, PT_ZMEAN_RP, PT_PERT_RP)
  write(*,*)  'GCR_PRE_dp'
...
!
      ...
      DO J=1,M
        DO I=1,N
            PT_sum_rp(I,J)= dble( PT_mean_rp%val) + dble( PT_zmean_rp(I,J)%val) +dble(PT_pert_rp(I,J)%val)
...
!COMPUTE OUTPUTED FIELDS ****************************************
    ...
    IF(KT/NPRINT*NPRINT.EQ.KT .OR. KT==6376/2) then
...
subroutine GCR_PRE10_RP(p_dp,pfx,pfy,hx,hy,s,S_full,b,p0,pb,e1,e2,cor,ip  &
              & ,d,q,r,ar,n1,n2,gc1,gc2, &
           &    MGH1IHX, MGH2IHY, AC, BC, AD, BD,  &
           &  niter,nitsm,icount,error, p_T, sum_time,&
           &  sum_lp_time,ID_PREC, codes, save_time , &
           & TIME, codesQ, IRHW, DX_rpe, DY_rpe, Exit_cond, EXP_NAME&
           & , iprint, num_of_bits, DP_Depth, Alp_REL, p_mean_rp, p_pert_rp)
use implicit_functions_rpe
USE rp_emulator
...
type(rpe_var) :: T_step_rp,r_rp(n1,n2),qr_rp(n1,n2), ar_rp(n1,n2),  A_c_rp(n1,n2), ps_rp(n1+1,n2), divi_rp(n1,n2)
type(rpe_var) :: PMB_rp(n1,n2), PMP0_rp(n1, n2), p_true_rp(n1, n2),p0_true_rp(n1, n2), b_true_rp(n1, n2) 
type(rpe_var) :: pfx_pert_rp(n1,n2),pfy_pert_rp(n1,n2), p_mean_rp(n1,n2)
logical :: restart
double precision, external :: norm 
...
!do it=1,52
!do j=1,n2
! write(*,*) j, p_pert_rp(:,j)%val, p_mean_dp(1,j)
! read(*,*)
!end do
!err0_dp=0.0d0
!err_true_dp=0.0d0
! DO J=1,n2
!   DO I=2,n1-1
!      err0_dp=err0_dp+(p_dp(I,J)-p_rp(I,J))*(p_dp(I,J)-p_rp(I,J))
!      err_true_dp=err_true_dp+(p_dp(I,J))*(p_dp(I,J))
!   enddo
! enddo
!    err0_dp=sqrt(err0_dp/((n1-2)*n2))
!    err_true_dp=sqrt(err_true_dp/((n1-2)*n2))
!    write(*,*) RPE_DEFAULT_SBITS, err0_dp,err_true_dp, (err0_dp)/err_true_dp
!end do
gc1_rp=gc1
gc2_rp=gc2
s_rp(:,:)=s(:,:)
...
p_T_rp(:,:)=p_T(:,:)
 ...
 DO J=1,n2
   DO I=1,n1
    ...
    !    PMP0_rp(I,J)= dble(p_rp(I,J)%val)-P0(I,J)  ! if r0 operator is split
...
elseif (ID_PREC==6) then
  call diagoc(T_step, A_c,a11,a21,s,n1,n2, DP_Depth)
elseif (ID_PREC==7) then
    call precon_prep_depth_rp(T_step_rp, A_c_rp, ps_rp, divi_rp,a11_rp,a12_rp,a21_rp,a22_rp,b11_rp,b22_rp,&
...
call laplfirst_depth1_dp(dble(p_mean_rp(:,:)%val),dble(r_HP_base_rp(:,:)%val), a11_dp,a12_dp,a21_dp,a22_dp,b11_dp,b22_dp,PMP0_dp,&
...
!     &                     pfx,pfy,S,n1,n2,IP, 23,DP_Depth)
!with Piotr's
!  CALL PRFORC_ABS_rp(p_rp(:,:),pfx_rp,pfy_rp,pb_rp,p0_rp, &
!       &      E1_rp(:,:),E2_rp(:,:),HX_rp,HY_rp,COR_rp,n1,n2,IP,GC1_rp,GC2_rp,Alp_REL_rp,1,1)
!  call diver_rp(r_HP_base_rp,pfx_rp,pfy_rp,hx_rp,hy_rp,s_rp,n1,n2,ip,-1)
RPE_DEFAULT_SBITS =10 !52
a11_rp(:,:)=a11_dp(:,:)
a21_rp(:,:)=a21_dp(:,:)
a12_rp(:,:)=a12_dp(:,:)
a22_rp(:,:)=a22_dp(:,:)
b11_rp(:,:)=b11_dp(:,:)
b22_rp(:,:)=b22_dp(:,:)
!call laplfirst_depth_rp(p_rp(:,:),r_HP_base_rp(:,:), a11_rp,a12_rp,a21_rp,a22_rp,b11_rp,b22_rp,PMP0_rp,&
!     &                     pfx_rp,pfy_rp,S_rp,n1,n2,IP, 23,DP_Depth)
RPE_DEFAULT_SBITS =10 !52
!p_pert_rp(:,:)=p_pert_rp(:,:)
call laplfirst_depth1_rp(p_pert_rp(:,:),r_HP_pert_rp(:,:), A11_rp,A12_rp,A21_rp,A22_rp,B11_rp,B22_rp &
          & ,PMP0_rp,pfx_pert_rp,pfy_pert_rp,s_rp,n1,n2,IP , num_of_bits,DP_Depth)
!RPE_DEFAULT_SBITS =10 !52
pfx_rp(:,:)=pfx_dp(:,:)+pfx_pert_rp(:,:)
pfy_rp(:,:)=pfy_dp(:,:)+pfy_pert_rp(:,:)
pfx_pert_rp(:,:)=pfx_pert_rp(:,:)
pfy_pert_rp(:,:)=pfy_pert_rp(:,:)
PMP0_rp(:,:)=PMP0_rp(:,:)
call laplfirst_depth2_rp(p_rp(:,:),r_HP_base_rp(:,:), a11_rp,a12_rp,a21_rp,a22_rp,b11_rp,b22_rp,PMP0_rp,&
     &                     pfx_rp,pfy_rp,S_rp,n1,n2,IP, 23,DP_Depth)
!PMP0_rp(:,:)=p_pert_rp(:,:)
!call laplfirst_depth2_rp(p_pert_rp(:,:),r_HP_pert_rp(:,:), A11_rp,A12_rp,A21_rp,A22_rp,B11_rp,B22_rp &
!          & ,PMP0_rp,pfx_pert_rp,pfy_pert_rp,s_rp,n1,n2,IP , num_of_bits,DP_Depth)

call laplfirst_depth3_rp(p_rp(:,:),r_HP_base_rp(:,:), a11_rp,a12_rp,a21_rp,a22_rp,b11_rp,b22_rp,PMP0_rp,&
     &                     pfx_rp,pfy_rp,S_rp,n1,n2,IP, 23,DP_Depth)
!call laplfirst_depth3_rp(p_pert_rp(:,:),r_HP_pert_rp(:,:), A11_rp,A12_rp,A21_rp,A22_rp,B11_rp,B22_rp &
!          & ,PMP0_rp,pfx_pert_rp,pfy_pert_rp,s_rp,n1,n2,IP , num_of_bits,DP_Depth)

  
!  CALL PRFORC_ABS_rp(p_pert_rp(:,:),pfx_rp,pfy_rp,pb_rp,p0_rp, &
!       &      E1_rp(:,:),E2_rp(:,:),HX_rp,HY_rp,COR_rp,n1,n2,IP,GC1_rp,GC2_rp,Alp_REL_rp,1,0)
!  call diver_rp(r_HP_pert_rp,pfx_rp,pfy_rp,hx_rp,hy_rp,s_rp,n1,n2,ip,-1)
!p_pert_rp(:,:)=p_pert_rp(:,:)
! call laplfirst_depth_rp(p_pert_rp(:,:),r_HP_pert_rp(:,:), A11_rp,A12_rp,A21_rp,A22_rp,B11_rp,B22_rp &
!          & ,PMP0_rp,pfx_rp,pfy_rp,s_rp,n1,n2,IP , num_of_bits,DP_Depth)
write(*,*) 'r0base, r0pert', sqrt(global_sqsums_rp(r_HP_base_rp/((n1-2)*n2),r_HP_base_rp,n1,n2,size_of_sum,2,N1-1,1,n2)) &
        & , sqrt(global_sqsums_rp(r_HP_pert_rp/((n1-2)*n2),r_HP_pert_rp,n1,n2,size_of_sum,2,N1-1,1,n2))
 DO J=1,n2
   DO I=1,n1
     r_HP_rp(I,J)=r_HP_base_rp(I,J)! +r_HP_pert_rp(I,J)
   enddo
 enddo
err0=rpe_0
PMB_rp(:,:)=PMB_rp(:,:)
DO J=1,n2
   DO I=1,n1
     r_HP_rp(I,J)=rpe_05*r_HP_rp(I,J)-(PMB_rp(I,J))
   enddo
 enddo
RPE_DEFAULT_SBITS =23 !52
! else
...
!   enddo
! enddo
!    err0_true_dp=sqrt(err0_true_dp/((n1-2)*n2))
!write(*,*) 'err0', err0_rp, err0_dp, sqrt(global_sqsums_rp(r_HP_rp/((n1-2)*n2),r_HP_rp,n1,n2,size_of_sum,2,N1-1,1,n2)),&
!               & err0_true_dp, abs(err0_rp -err0_dp)/err0_dp, &
! & abs(sqrt(global_sqsums_rp(r_HP_rp/((n1-2)*n2),r_HP_rp,n1,n2,size_of_sum,2,N1-1,1,n2)) -err0_dp)/err0_dp, counter
!read(*,*)
...
!if(comp_with_dp) then
 DO J=1,n2
   DO I=1,n1
     r_HP_dp(I,J)=0.5d0*r_HP_dp(I,J)-(p_dp(I,J)-dble(b(I,J)))
   enddo
 enddo
  r0_HP_dp(:,:)=r_HP_dp(:,:)
!        write(*,*) 'r0 full', norm(r_HP(:,:),r_HP_dp(:,:),n1,n2,1,n1,1,n2,2), &
...
!endif
 IF(RPE_DEFAULT_SBITS < 52) then
    err0_rp=sqrt(global_sqsums_rp(r_HP_rp/((n1-2)*n2),r_HP_rp,n1,n2,size_of_sum,2,N1-1,1,n2))
  endif
    errnm1=err0_rp


if (iprint==1) then
    call write_residual(dble(r_HP_rp%val),dble(eps*Exit_cond), niter, dble(TIME),&
           &    codesQ, codes, IRHW,dble( DX_rpe), dble(DY_rpe),&
                     & n1, n2, num_of_bits, 10 ,EXP_NAME)
endif
...
!call precon_rp(r_HP_rp,x_rp(:,:,1),ax_rp(:,:,1), T_step_rp,  A_c_rp, ps_rp, divi_rp,a11_rp&
!         & ,a12_rp,a21_rp,a22_rp,b11_rp,b22_rp,p0_rp,  &
...
qrr0=sqrt(global_sqsums(x(:,:,1),x(:,:,1),n1,n2,size_of_sum,2,N1-1,1,n2))
!endif
  call lapl_depth_rp(x_rp(:,:,1),ax_rp(:,:,1), A11_rp,A12_rp,A21_rp,A22_rp &
           & ,B11_rp,B22_rp,P0_rp,pfx_rp,pfy_rp,s_rp,n1,n2,IP,num_of_bits,DP_Depth)
...
write(*,*) 'L(q0) part1',  norm(ax_rp(:,:,1),ax_dp(:,:,1),n1,n2,1,n1,1,n2,2), &
                  & norm(ax_rp(:,:,1),ax_dp(:,:,1),n1,n2,1,n1,1,n2,1) 
...
!if(comp_with_dp) then
      ...
      ax_dp(I,J,1)=0.5d0*ax_dp(I,J,1)-x_dp(I,J,1)
        enddo
      enddo
write(*,*) 'L(q0)',  norm(ax_rp(:,:,1),ax_dp(:,:,1),n1,n2,1,n1,1,n2,2), &
                  & norm(ax_rp(:,:,1),ax_dp(:,:,1),n1,n2,1,n1,1,n2,1) 
!endif
   !! end of rewrite
...
!write(*,*) 'beta',l,beta_rp, beta_dp, &
!& -global_sqsums_rp(r_HP_rp(:,:),ax_rp(:,:,l),n1,n2,size_of_sum,2,N1-1,1,n2)/  &
!& global_sqsums_rp(ax_rp(:,:,l),ax_rp(:,:,l),n1,n2,size_of_sum,2,N1-1,1,n2),  &
!               & abs(beta_rp -beta_dp)/beta_dp, &
! & abs(-global_sqsums_rp(r_HP_rp(:,:),ax_rp(:,:,l),n1,n2,size_of_sum,2,N1-1,1,n2)/  &
!& global_sqsums_rp(ax_rp(:,:,l),ax_rp(:,:,l),n1,n2,size_of_sum,2,N1-1,1,n2) -beta_dp)/beta_dp
!endif
 IF(RPE_DEFAULT_SBITS < 52) then
 ! IF(global_sum_fix) then
...
!endif
 ...
 IF(restart) then
!  CALL PRFORC_ABS_rp(p_pert_rp(:,:)+p_T_rp(:,:),pfx_rp,pfy_rp,pb_rp,p0_rp, &
!       &      E1_rp(:,:),E2_rp(:,:),HX_rp,HY_rp,COR_rp,n1,n2,IP,GC1_rp,GC2_rp,Alp_REL_rp,1,0)
  call diver_rp(r_HP_pert_rp,pfx_rp,pfy_rp,hx_rp,hy_rp,s_rp,n1,n2,ip,-1)
!write(*,*) 'rnbase, rnpert', sqrt(global_sqsums_rp(r_HP_base_rp/((n1-2)*n2),r_HP_base_rp,n1,n2,size_of_sum,2,N1-1,1,n2)) &
!        & , sqrt(global_sqsums_rp(r_HP_pert_rp/((n1-2)*n2),r_HP_pert_rp,n1,n2,size_of_sum,2,N1-1,1,n2))
 ...
 DO J=1,n2
   DO I=1,n1
     r_HP_rp(I,J)=rpe_05*r_HP_rp(I,J)-(PMB_rp(I,J)) -p_T_rp(I,J)
   enddo
 enddo
...
!if(comp_with_dp) then
    errn_dp=0.0d0
      DO J=1,n2
        ...
        enddo
      enddo
      errn_dp= errn_dp/((n1-2)*n2)
!write(*,*) 'errn', sqrt(errn_rp), sqrt(errn_dp), &
!& sqrt(global_sqsums_rp(r_HP_rp/((n1-2)*n2),r_HP_rp,n1,n2,size_of_sum,2,N1-1,1,n2)),&
!               & abs(sqrt(errn) -sqrt(errn_dp))/sqrt(errn_dp), &
! & abs(sqrt(global_sqsums_rp(r_HP_rp/((n1-2)*n2),r_HP_rp,n1,n2,size_of_sum,2,N1-1,1,n2)) -sqrt(errn_dp))/sqrt(errn_dp)
!endif
 IF(RPE_DEFAULT_SBITS < 52) then
!  IF(global_sum_fix) then
    errn_rp      =global_sqsums_rp(r_HP_rp/((n1-2)*n2),r_HP_rp,n1,n2,size_of_sum,2,N1-1,1,n2)
 endif
    write(*,*) 'errn fin', sqrt(errn_rp)
    !  endif
...
!!! begin true residual
r_true_dp(:,:)=0.0d0
  CALL PRFORC_ABS_dp(dble(p_true_dp(:,:))+dble(p_T_rp(:,:)%val),pfx_dp,pfy_dp,dble(pb),dble(p0), &
       &      dble(E1(:,:)),dble(E2(:,:)),dble(HX),dble(HY),dble(COR), &
       &      n1,n2,IP,dble(GC1),dble(GC2),dble(Alp_REL),1,1)
  call diver_dp(r_true_dp,pfx_dp,pfy_dp,dble(hx),dble(hy),dble(s),n1,n2,ip,-1)
...
DO J=1,n2
  DO I=1,n1
    r_true_dp(I,J)=0.5d0*r_true_dp(I,J)-(dble(p_true_dp(I,J))+dble(p_T_rp(I,J)%val)-dble(b_true(I,J)))
 ! write(*,*), i, J, P(i,J)
  enddo
enddo
    err_true_dp=0.0d0
...
write(*,*) 'True Residual', err_true_dp, err0_dp, err_true_dp/err0_dp 
!!! end true residual
...
!read(*,*)
  ...
  ! read(*,*)
   ...
   write(*,*) 'exiting 1', exiting 
    if(exiting .eqv. .true.) exit
    if(errn_rp.ge.errnm1) exiting=.true.
    write(*,*) errn_rp, errnm1
    errnm1=errn_rp
   write(*,*) 'exiting 2', exiting 
...
!endif
 ...
 call lapl_depth_rp(qu_rp(:,:),aqu_rp(:,:), A11_rp,A12_rp,A21_rp,A22_rp,B11_rp,B22_rp &
...
RPE_DEFAULT_SBITS =23 !52
  ! double precision base
   call laplfirst_depth1_dp(dble(p_mean_rp(:,:)%val),dble(r_HP_base_rp(:,:)%val), &
...
!!! begin true residual
   ...
   DO J=1,n2
     DO I=1,n1
       r_true_dp(I,J)=0.5d0*r_true_dp(I,J)-(dble(p_T_rp(I,J)%val)+dble(b_true(I,J)))
       ! write(*,*), i, J, P(i,J)
     enddo
   enddo
   err_true_dp=0.0d0
...
implicit none
type(rpe_var) :: p(n,m),s(n,m)
type(rpe_var) :: S_full, cnst
INTEGER :: N, M
...
implicit none
      REAL(Kind=4) :: h0(514,256),x(n),y(m)
      REAL(Kind=4) :: tp0(512,256),tp1(256,128),flon,flat, pi
  ...
  IP0(I)=MOD(I+(N-2)/2-1,N-2)+1
   ...
   if (High_res_smooth) then
      ...
      DO KIT=1,KITER
        ...
        enddo
        do i=1,514
         fl(i,1)=0.25*(scr(i,2)+2.*scr(i,1)+scr(ip1(i),1))
         fl(i,256)=0.25*(scr(ip1(i),256)+2.*scr(i,256)+scr(i,256-1))
        enddo
      ENDDO
      ...
      do j=1,M
       do i=1,N
         FL_Coarse(i,j)=0.5*( fl(i*coarse,(j-1)*coarse+1)+  fl(i*coarse,(j-1)*coarse+2) )
        ! tp2(i,j)=0.5*( tp0(i*coarse,(j-1)*coarse+2)+  tp0(i*coarse,(j-1)*coarse+3) )
         
       enddo
       call XBC(FL_Coarse,N,M)
       !write(*,*) fl_Coarse(:,j)
       !read(*,*)
      enddo
      ...
      DO KIT=1,KITER
        do j=1,M
         do i=2,N-1
          scr(i,j)=0.25*(fl_Coarse(i+1,j)+2.*fl_Coarse(i,j)+fl_Coarse(i-1,j))
         enddo
          scr(1,j)=scr(N-1,j)
          scr(N,j)=scr(2,j)
        enddo
        do j=2,M-1
         do i=1,N
          fl_Coarse(i,j)=0.25*(scr(i,j+1)+2.*scr(i,j)+scr(i,j-1))
         enddo
        enddo
        do i=1,N
         fl_Coarse(i,1)=0.25*(scr(i,2)+2.*scr(i,1)+scr(ip0(i),1))
         fl_Coarse(i,M)=0.25*(scr(ip0(i),M)+2.*scr(i,M)+scr(i,M-1))
        enddo
      ENDDO
