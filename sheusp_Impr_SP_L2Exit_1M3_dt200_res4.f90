...
S_full_DP=0.0d0
  ...
  DO J=1,M
    DO I=2,N-1
      S_full_DP=S_full_DP+S_23(I,J)
    end do
  end do

  write(*,*) 'S_full', S_full,S_full_DP,global_sums(S_23,n,m,512,2,N-1,1,M)
  IF(global_sum_fix) then
  S_full=global_sums(S_23,n,m,size_of_sum,2,N-1,1,M)
  endif
...
REAL(Kind=4) :: p(n1,n2),pfx(n1,n2),pfy(n1,n2),hx(n1,n2),hy(n1,n2),s(n1,n2), &
     ...
     &   p_T(n1,n2), r_true(n1,n2), r0_true(n1,n2), p_true(n1,n2), &
...
err0=rpe_0
 ...
 enddo
!err0=global_sums(r_HP,n,m,size_of_sum,2,N-1,1,M)
...
do it=1,itr
 ...
 If (it==1000) then
  ...
  do l=1,lord
    ...
    DO J=1,n2
      DO I=2,n1-1
        rax_dp=rax_dp+r(I,J)*ax(I,J,l)
        ax2_dp(l)=ax2_dp(l)+ax(I,J,l)*ax(I,J,l)
      enddo
    enddo
...
& global_sqsums(ax(:,:,l),ax(:,:,l),n1,n2,size_of_sum,2,N1-1,1,n2) -beta_dp)/beta_dp
  ...
  IF(global_sum_fix) then
    rax      =global_sqsums(r(:,:),ax(:,:,l),n1,n2,size_of_sum,2,N1-1,1,n2)
...
!23 !!was 23 before exit criterium   
    ...
    errn_dp=0.0d0
      DO J=1,n2
        DO I=2,n1-1
         errn_dp=errn_dp+r_HP(I,J)*r_HP(I,J)
        enddo
      enddo
...
& sqrt(global_sqsums(r_HP,r_HP,n1,n2,size_of_sum,2,N1-1,1,n2)),&
 ...
 & abs(sqrt(global_sqsums(r_HP,r_HP,n1,n2,size_of_sum,2,N1-1,1,n2)) -sqrt(errn_dp))/sqrt(errn_dp)
  IF(global_sum_fix) then
    errn      =sqrt(global_sqsums(r_HP,r_HP,n1,n2,size_of_sum,2,N1-1,1,n2))
...
!write(*,*) 'second precon', it
 ...
 !     del(ll)=max(del(ll),rpe_05)
    ...
    do ll=1,l
      ...
      DO J=1,n2
        DO I=2,n1-1
          axaqu_dp(ll)=axaqu_dp(ll)+ax(I,J,ll)*aqu(I,J)
        enddo
      enddo
