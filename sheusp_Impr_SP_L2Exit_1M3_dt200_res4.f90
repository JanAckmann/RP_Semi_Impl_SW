...
!--->                CORIOLIS AND METRIC FORCES
 ...
 call add_NewForcesA_dp(QX_HP, QY_HP, U_dp(:,:,0), V_dp(:,:,0), E1_dp(:,:,:), E2_dp(:,:,:) &
   &,dble(COR),GC1_dp, GC2_dp, Alp_REL_dp,QXS_dp, QYS_dp, N, M)
! call add_NewForcesA_dp(QX_HP, QY_HP, U_dp(:,:,0),  V_dp(:,:,0), E1_dp(:,:,:), E2_dp(:,:,:) &
...
!if (iprint==1) then
!    call write_residual(REAL(r_true_dp),REAL(eps*Exit_cond), niter+1,REAL( TIME), &
!            &  codesQ, codes, IRHW, REAL(DX_rpe),REAL( DY_rpe), n1, n2, num_of_bits, 5 ,EXP_NAME)
!endif
