...
do ID_PREC=7,7,-5
 ...
 ! read(*,*)
  ...
  !ID_PREC=0
   EXP_NAME= 'data/sheusp_DP_L2Exit_1M3_dt800_res1'
...
elseif (IRHW.EQ.3)then
  CALL Earthtopo(P0_512x256,X_23,Y_23,N,M)
...
implicit none
      DOUBLE PRECISION :: h0(514,256),x(n),y(m)
      DOUBLE PRECISION :: tp0(512,256),tp1(128,64),flon,flat, pi
      ...
      implicit none
      DOUBLE PRECISION :: FL(514,256),SCR(514,256),FL_Coarse(N,M)
      INTEGER :: IP0(N),IP1(514), N, M

      logical :: High_res_smooth
      DOUBLE PRECISION :: h0mx, h0mn
      INTEGER :: kiter, kit, I, J, coarse
...
DO I=1,N
  IP0(I)=MOD(I+(N-2)/2-1,N-2)+1
   ...
   if (High_res_smooth) then
      ...
      do j=1,M
       do i=1,N
         FL_Coarse(i,j)=0.5*( fl(i*coarse,(j-1)*coarse+2)+  fl(i*coarse,(j-1)*coarse+3) )
        ! tp2(i,j)=0.5*( tp0(i*coarse,(j-1)*coarse+2)+  tp0(i*coarse,(j-1)*coarse+3) )
      ...
      do j=1,M
       do i=1,N
        !fl_coarse(i,j)=1.268678*fl_coarse(i,j)
        fl_coarse(i,j)=1.40*fl_coarse(i,j)
        !fl_coarse(i,j)=1.349678*fl_coarse(i,j)
        h0mx=max(h0mx,fl_coarse(i,j))
        h0mn=min(h0mn,fl_coarse(i,j))
       enddo
