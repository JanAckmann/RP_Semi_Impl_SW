...
N=mult*32*2+2
M=mult*32
write(*,*) 'Resolution', mult
call run_resolution(N,M)
enddo

contains

subroutine run_resolution (N,M)
implicit none
...
REAL(kind=HP):: deti, det40, det41, det42, det43, det44, det3_ARM_RP,   &
                           & d11, d12, d13, d14, d21, d22, d23, d24,       &
                           & d31, d32, d33, d34, d41, d42, d43, d44,       &
                           & s1, s2, s3, s4
REAL(kind=HP):: T_step, Delta_t_I, dn, dni, util, vtil, swcp, max_QX_QY
...
implicit none
integer, parameter :: HP=8
REAL(kind=4)::  S_full, start_time, end_time
REAL(Kind=HP) :: S(M,N)
...
CALL XBC_T_rp(V,M,N)
  ...
  DO I=2,N-1
DO J=2,M-1
    F(J,I)= (U(J,I+1)-U(J,I-1)+V(J+1,I)-V(J-1,I))/S(J,I)
  end do
end do    
 
DO I=2,N-1
  F(1,I)= (U(1,I+1)-U(1,I-1)+(V(2,I)+V(1,I)))/S(1,I)
  F(M,I)= (U(M,I+1)-U(M,I-1)-(V(M,I)+V(M-1,I)))/S(M,I)
ENDDO
CALL XBC_T_rp(F,M,N)
