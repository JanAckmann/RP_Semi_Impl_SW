...
c     PARAMETER(NLON=258,NLAT=128)
     ...
     1     QX(N,M),QY(N,M),PT(N,M),QXS(N,M),QYS(N,M),
...
c     DATA USCAL,H00,HMTS/20.,8.E3,2.E3/
      DATA USCAL,H00,HMTS/20.,8.E3,1.0/
c 1 day = 2160*40s
...
c     DATA DT,NT,NPRINT,NPLOT/200,6376,797,1594/ !equivalent for 512x256
      DATA DT,NT,NPRINT,NPLOT/200,6376,797,1594/ !equivalent for 512x256
...
C--->         CORIOLIS, METRIC & ABSORBING FORCES
      ...
      ENDIF
      DO 139 I=1,NM
      UA(I,1)=QX(I,1)+.5*(2.*E1(I,1,0)-E1(I,1,-1))+.5*ALP(I,1)*QXS(I,1)
      VA(I,1)=QY(I,1)+.5*(2.*E2(I,1,0)-E2(I,1,-1))+.5*ALP(I,1)*QYS(I,1)
...
COMPUTE OUTPUTED FIELDS ****************************************
     ...
     &    CALL DIAGNOS(U,V,PD,PT,COR,HX,HY,S,X,XX,Y,IP,PI2,KT,1)
        ...
        ENDIF
        !IF(KT/NPRINT*NPRINT.EQ.KT) read(*,*)
      ENDIF
  100 CONTINUE
...
c mountain shape functions
      dist(xln,ylt) =2.*sqrt( (cos(ylt)*sin((xln-x0)/2))**2
     .                      +       (sin((ylt-y0)/2))**2 ) !/xl
...
!     print*, i,j,flon,flat,tp0(i,j)
    ...
    1 continue
      ...
      do 2 j=1,m
      do 2 i=1,n
        h0mx=amax1(h0mx,h0(i,j))
        h0mn=amin1(h0mn,h0(i,j))
    2 continue
      print*, h0mx,h0mn
      cycmx1=-1.e15
      cycmxn=-1.e15
    ...
    3 continue
      ...
      do j=1,m
      do i=1,n
        h0mx=amax1(h0mx,fl(i,j))
        h0mn=amin1(h0mn,fl(i,j))
      enddo
      enddo
      print*, h0mx,h0mn
...
!       if (y0 < 0.) y2 = exp(-(y0-ymin)/abswidth)
      ...
      DIMENSION FL(N,M),FLS(N,M),SCR1(N,M),SCR2(N,M),IP(N)
