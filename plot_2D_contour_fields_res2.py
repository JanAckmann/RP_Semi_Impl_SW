...
pathlist= ['../data/sheusp_DP_L2Exit_1M3_dt400_res2/'] # '../data/data_ADI_NOTgcr_D6_2M4_RP_res4/'
          ...
          for var in varlist: 
            ...
            if (exp == '1'):
             ...
             plt.contour(grid_var, 20, extent=(xcoord.min(), xcoord.max(), 
               ycoord.min(), ycoord.max()),
               interpolation='None', colors='black', vmin=np.amin((grid_var)), vmax=np.amax(grid_var))
            q= plt.quiver(grid_xcoord[::8, ::8],grid_ycoord[::8, ::8], grid_varU[::8, ::8],grid_varV[::8, ::8], pivot='mid')
            if (exp == '1'):
