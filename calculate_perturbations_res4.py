...
for path in pathlist: 
 for time in times:
  for exp in exp_list:
    ...
    kinetic=Kinetic_Energy(abs(grid_varH-grid_varH_gen), grid_varU-grid_varU_gen, grid_varV-grid_varV_gen, sorted(set(ycoord)), ncols, nrows)
    kinetic_gen=Kinetic_Energy(grid_varH_gen, grid_varU_gen, grid_varV_gen, sorted(set(ycoord)), ncols, nrows)
    print('kinetic/kinetic_gen')
    print((L2_norm2D(kinetic, ncols, nrows)/L2_norm2D(kinetic_gen, ncols, nrows)))# ,Linf_norm(kinetic-kinetic_gen)/ Linf_norm(kinetic_gen))
    ...
    print(Linf_norm2D(kinetic, ncols, nrows)/Linf_norm2D(kinetic_gen, ncols, nrows))# ,Linf_norm(kinetic-kinetic_gen)/ Linf_norm(kinetic_gen))
  
    #print(Linf_norm2D(kinetic/kinetic_gen, ncols, nrows)) #/Linf_norm2D(kinetic_gen, ncols, nrows))# ,Linf_norm(kinetic-kinetic_gen)/ Linf_norm(kinetic_gen))
