...
do ID_PREC=7,7,-5
 do DP_Depth_nr=1,3,1
  ...
  !ID_PREC=0
EXP_NAME= 'data/sheusp_IMPR_SP_FRPPreAll_FRPLap_SPr0V2_RPxAx_DP'//trim(adjustl(Dp_depth_str))//'_L2Exit_1M3_dt800_res1'
...
mpfl=999999
elseif(IRHW==1) then
NT = 6480/4 !  6376 !int(6376*(200.0/240.0)) !12960  
...
IF(IANAL.EQ.0) THEN
  DO KT=1,NT
    ...
    IPRINT=0
    IF(KT/NPRINT*NPRINT.EQ.KT .OR. KT==6376/4) IPRINT=1
    !if (IPRINT==1) then
...
!COMPUTE OUTPUTED FIELDS ****************************************
    ...
    IF(KT/NPRINT*NPRINT.EQ.KT .OR. KT==6376/4) then
...
DO I=1,N
  IP0(I)=MOD(I+(N-2)/2-1,N-2)+1
   ...
   if (High_res_smooth) then
      ...
      do j=1,M
       do i=1,N
         FL_Coarse_dp(i,j)=0.5*( fl(i*coarse,(j-1)*coarse+2)+  fl(i*coarse,(j-1)*coarse+3) )
        ! tp2(i,j)=0.5*( tp0(i*coarse,(j-1)*coarse+2)+  tp0(i*coarse,(j-1)*coarse+3) )
      ...
      do j=1,M
       do i=1,N
        ...
        fl_coarse(i,j)=1.40*fl_coarse_dp(i,j)
